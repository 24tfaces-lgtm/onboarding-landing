// === Welcome Pack Player: список слайдов (глобально) ===
window.SLIDES = [
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!img || !thumbs) {
    console.warn("Не найдены контейнеры плеера. Проверь разметку welcome-pack.html");
    return;
  }
  ...
  function render() {
    ...
    const s = SLIDES[i];
    img.src = s.src;
    img.alt = s.title || `Слайд ${i+1}`;
    img.onerror = () => console.warn("Не удалось загрузить изображение:", s.src);
  ...
  // Инициализация
  if (!SLIDES.length) {
    console.warn("SLIDES пуст. Проверь ссылки/ветку/кэш.");
